...
# modified by Calros Buen Abad to ameliorate the amplification bias of different isoforms
# Previous version could result in large discrepancies in amplification biases that ocassionally resulted 
# in large disparities between isoforms from the same gene.
# The fact that isoforms share the majority of their sequence makes these scenarios not likely to be realistic.
True2ObservedCounts <- function(SE=NULL,true_counts,meta_cell,protocol,alpha_mean=0.1,alpha_sd=0.002,
  ...
  amp_bias <- cal_amp_bias(lenslope, nbins, gene_len, amp_bias_limit) #comment this out in biased sims
  ...
  observed_counts <- lapply(c(1:ncells),function(icell){
    #amp_bias <- cal_amp_bias(lenslope, nbins, gene_len, amp_bias_limit) # Added amplification bias here for biased sims
...
cal_amp_bias <- function(lenslope, nbins, gene_len, amp_bias_limit){
  ...
  amp_bias <- len_bias # removed random bias for non-bias sims
  print('only length bias')
  return(amp_bias)
}
