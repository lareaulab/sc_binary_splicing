...
for (sim in 26:50) {
    ...
    lapply(gene_len, write, paste('sim2_extended/gene_length', suffix1, 'txt', sep='.'), append=TRUE, ncolumns=1000)

    write.table(mrna$cell_meta, paste('sim2_extended/meta_cell', suffix1, 'tab', sep='.'), 
  ...
  for (capture in seq(from = 0.01, to = 0.25, by=0.01)){
        ...
        write.table(mrna_matrix, paste('sim2_extended/true_counts', suffix, 'tab', sep='.'), 
                    quote = FALSE, row.names = FALSE, col.names = FALSE, sep='\t')

        write.table(observed_reads[[1]], paste('sim2_extended/observed_counts', suffix, 'tab', sep='.'), 
                    quote = FALSE, row.names = FALSE, col.names = FALSE, sep='\t')
