...
foreach(sim = 1:30, .packages = c('knitr', 'fAsianOptions', 'devtools')) %dopar% {
  ...
  for (pcr2 in 5:15){
        ...
        observed_reads <- True2ObservedCounts(true_counts=mrna_matrix, meta_cell=mrna$cell_meta, protocol="nonUMI", alpha_sd=0.002,                                                       gene_len=gene_len, depth_mean=1e5, depth_sd=3e4, lenslope=0.01,
                                              amp_bias_limit=c(-0.1, 0.1), nPCR1=pcr1, nPCR2=pcr2)
