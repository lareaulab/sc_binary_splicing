...
for k in [7, 12, 17, 22, 27, 32, 37, 50, 100, 150, 200, 229, 234, 239, 244, 249, 254, 259, 488]:
#for k in [17, 22, 27, 239, 244, 249, 254, 488]:
    ...
    chen_mock_dict = get_mock_dict(chen_PSI.loc[test_exons], chen_norm_PSI.loc[test_exons], W_chen, mock=1000)
    ...
    pgamma_chen.to_csv('autocorrelation_k_test/pgamma_chen_'+str(k)+'.tab', sep='\t', header=True, index=True)
...
for k in [5, 10, 15, 20, 25, 30, 50, 100, 105, 110, 115, 120, 125, 130, 230]:
#for k in [10, 15, 20, 110, 115, 120, 230]:
    ...
    trapnell_mock_dict = get_mock_dict(trapnell_PSI.loc[test_exons], trapnell_norm_PSI.loc[test_exons], W_trapnell, mock=1000)
    ...
    pgamma_trapnell.to_csv('autocorrelation_k_test/pgamma_trapnell_'+str(k)+'.tab', sep='\t', header=True, index=True)
...
for k in [4, 9, 14, 19, 24, 29, 50, 86, 91, 96, 101, 106, 111, 116, 202]:
#for k in [9, 14, 19, 96, 101, 106, 202]:
    song_norm_PSI = get_signature_matrix(song_PSI)
    W_song = get_distance_matrix(song_pca_clust, k=k)
    ...
    pgamma_song.to_csv('autocorrelation_k_test/pgamma_song_'+str(k)+'.tab', sep='\t', header=True, index=True)
